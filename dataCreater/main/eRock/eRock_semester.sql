...
(
   que_id               numeric(10,0) not null comment '题目编号',
   que_content          varchar(60) comment '题目内容',
   primary key (que_id)
);
...
(
   oa_id                numeric(10,0) not null comment '答案ID',
   que_id               numeric(10,0) comment '题目编号',
   oa_content           varchar(60) comment '答案内容',
...
(
   aa_id                numeric(40,0) not null comment '考勤ID',
   stu_id               varchar(30) comment '学生学号',
...
(
   ...
   se_type              varchar(40) comment '评价类型',
...
(
   ...
   tea_id               varchar(30) comment '老师工号',
   sg_id                numeric(10,0) comment '小组ID',
   cr_id                numeric(40,0) comment '课堂ID',
...
(
   ...
   et_create_time       date comment '创建时间',
   primary key (et_id)
);
...
(
   com_r_id             numeric(40,0) not null comment '记录ID',
   com_r_score          varchar(40) comment '记录比分',
   session_time         date comment '场次时间',
   specification        varchar(10) comment '比赛规格',
   com_r_name           varchar(60) comment '比赛名称',
   primary key (com_r_id)
);
...
(
   ...
   com_r_id             numeric(40,0) comment '记录ID',
   stu_id               varchar(30) comment '学生学号',
...
(
   ...
   finish_date          date comment '学期结束日期',
...
(
   sg_id                numeric(10,0) not null comment '小组ID',
   sg_name              varchar(60) comment '小组名',
   sg_leader            numeric(40,0) comment '小组组长',
   primary key (sg_id)
);
...
alter table a1_result add constraint "FK_A1 q to o" foreign key (que_id)
      references a1_question (que_id) on delete restrict on update restrict;
...
alter table a2_teaching_assistant_evaluation add constraint "FK_A2 g to te" foreign key (sg_id)
      references stu_group (sg_id) on delete restrict on update restrict;
...
alter table a_mark_sheet add constraint "FK_class_register to ms" foreign key (cr_id)
      references class_register (cr_id) on delete restrict on update restrict;
...
alter table c2_personnel_sheet add constraint "FK_C2 cr to ps" foreign key (com_r_id)
      references c2_competition_record (com_r_id) on delete restrict on update restrict;
...
alter table student add constraint "FK_group to stu" foreign key (sg_id)
      references stu_group (sg_id) on delete restrict on update restrict;
